...
class DvrView(QWidget):
    ...
    def initUI(self):
        ...
        self.no_video_label.setStyleSheet('color: #2f1188; font-size: 30px;')
    ...
    def riproduction(self, panel):
        if self.current_playing_panel and self.current_playing_panel != panel:
            ...
            print(f'\n{self.current_playing_panel.file_video.name} is stopping: {self.current_playing_panel.is_playing}')
        self.current_playing_panel = panel if panel.is_playing else None
        print(f'\n{panel.file_video.name} is playing: {panel.is_playing}')
        if panel.is_playing:
